...
class DemolishRails(BuildingMode):
    def __init__(self, game_state: "GameState"):
        self.game_state = game_state
                ...
                if map.is_point_on_map(to_point):
                    ...
                    for edge in edges:
                        if edge in map.placed_rails:
                            Rail(edge).render(graphics, color=Colors.DESTROYING_TRACK)
...
class BuildSignals(BuildingMode):
    ...
    def render(self, graphics: GraphicsContext):
        ...
        map = self.game_state.map
        if rail is not None and map.is_point_on_map(rail.edge.to_point) and map.is_point_on_map(rail.edge.from_point):
